...
// Componente para links protegidos
const ProtectedNavLink = ({ to, feature, children, hasAccess, onClick }) => {
    const handleClick = (e) => {
        if (!hasAccess) {
            e.preventDefault();
            alert('Esta funcionalidade requer plano Premium. Acesse "Planos" para fazer upgrade.');
            return;
        }
        if (onClick) onClick();
    };
    ...
    return (
        <div 
            className={`nav-link-wrapper ${!hasAccess ? 'disabled-link' : ''}`}
            style={{
        ...
        >
            <NavLink 
                to={hasAccess ? to : '#'} 
                className={({ isActive }) => (isActive && hasAccess ? 'active' : '')}
            ...
            <nav className="main-navbar">
                ...
                {!isAdmin && (
                    <div className={`navbar-center ${isMobileMenuOpen ? 'mobile-active' : ''}`}>
                        ...
                        <ProtectedNavLink to="/cadastro" feature="cadastro" hasAccess={hasAccess('cadastro')} onClick={closeMobileMenu}>
                            <Icons.Users /> Cadastro
                        </ProtectedNavLink>
                        <ProtectedNavLink to="/relatorios" feature="relatorios" hasAccess={hasAccess('relatorios')} onClick={closeMobileMenu}>
                            <Icons.Chart /> Relatórios
                        </ProtectedNavLink>
                        <NavLink to="/profile" className={({ isActive }) => (isActive ? 'active' : '')} onClick={closeMobileMenu}>
                            <Icons.Profile /> Perfil
                        </NavLink>
                        <NavLink to="/planos" className={({ isActive }) => (isActive ? 'active' : '')} onClick={closeMobileMenu}>
