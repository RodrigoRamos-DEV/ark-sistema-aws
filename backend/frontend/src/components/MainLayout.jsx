...
const ExpiryOverlay = ({ onLogout }) => {
    const whatsappNumber = "+55 21 97304-4415";
    
    const handleWhatsAppClick = () => {
        const user = JSON.parse(localStorage.getItem('user') || '{}');
        const companyName = user.companyName || 'Não informado';
        const message = encodeURIComponent(
            `Olá! Minha licença do Sistema ARK expirou. Gostaria de renovar. Empresa: ${companyName}`
        );
        ...
        <div className="expiry-overlay">
            <div className="expiry-overlay-content">
                <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                    ...
                    <h2 style={{ color: '#ff4444', marginBottom: '15px' }}>Sistema Vencido</h2>
                    <p style={{ fontSize: '16px', lineHeight: '1.5', marginBottom: '30px' }}>
                        A sua licença de acesso ao sistema expirou. Por favor, entre em contacto com o suporte para regularizar a sua situação e reativar o acesso.
                    </p>
                </div>
                ...
                <div style={{ marginTop: '30px', textAlign: 'center', fontSize: '14px', color: '#666' }}>
                    <p>📞 Suporte: {whatsappNumber}</p>
                    <p>💬 Clique no botão do WhatsApp para contato direto</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div>
            ...
            {licenseStatus.status === 'expired' ? (
                <ExpiryOverlay onLogout={handleLogout} />
            ) : (
